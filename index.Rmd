...
Mex_Data_COVID <- read.csv(unz(temp, "201031COVID19MEXICO.csv"))
unlink(temp); rm(temp)
```
...
ggsave("Casos_estados_20201030.jpg",plot_casos_estados, dpi = 500, width = 13, height = 9)
...
Def_edad_Tlx_meses_plot <- plot_ly(def_mes_Tlx, type = 'box') %>% 
  ...
  add_trace(x =~mes[def_mes_Tlx$sexo == 'Mujer'],
            ...
            meanline = list(visible = T),
            color = I("pink")) %>%
  layout(yaxis = list(zeroline = F),boxmode = 'group',
         title = ~paste("<b>Distribución de fallecimientos por sexo y edad en Tlaxcala</b>"),
         titlefont = list(color = 'darkred'));Def_edad_Tlx_meses_plot
