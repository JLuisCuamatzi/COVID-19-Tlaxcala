...
Mex_Data_COVID <- read.csv(unz(temp, "201030COVID19MEXICO.csv"))
unlink(temp); rm(temp)
```
...
plot_casos <- ggplot(mn_Mx_2, aes(x = as.Date(fecha_sintomas), y = casos, fill = tipo_paciente))+
  ...
  theme(axis.text.x = element_text(face = "bold", color = "#993333",size = 8, angle = 90),
        ...
        plot.title = element_text(face = "bold", color = "red", size = 14, hjust = 0.5))+
  scale_x_date(date_breaks = "months" , date_labels = "%b")+
  annotate(geom="text", x=as.Date("2020-04-30"), y=8000, label= c("Ambulatorio =                                ",n_amb), color="red") +
  annotate(geom="text", x=as.Date("2020-04-30"), y=7500, label= c("Hospitalizado =                                  ",n_hosp), color="red");ggplotly(plot_casos)
...
plot_decesos <- ggplot(Decesos_Mx_2, aes(x = as.Date(fecha_def), y = casos, fill = tipo_paciente))+
  ...
  theme(axis.text.x = element_text(face = "bold", color = "#993333",size = 8, angle = 90),
        ...
        plot.title = element_text(face = "bold", color = "red", size = 14, hjust = 0.5))+
  scale_x_date(date_breaks = "months" , date_labels = "%b")+
  annotate(geom="text", x=as.Date("2020-05-25"), y=600, label= c("Ambulatorio =                                ",n_amb), color="red") +
  annotate(geom="text", x=as.Date("2020-05-25"), y=550, label= c("Hospitalizado =                                  ",n_hosp), color="red"); ggplotly(plot_decesos)
