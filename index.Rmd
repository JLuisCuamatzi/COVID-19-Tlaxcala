...
Pos_Mx_Enero <- Pos_Mx[grep("-01-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Febrero <- Pos_Mx[grep("-02-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Marzo <- Pos_Mx[grep("-03-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Abril <- Pos_Mx[grep("-04-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Mayo <- Pos_Mx[grep("-05-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Junio <- Pos_Mx[grep("-06-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Julio <- Pos_Mx[grep("-07-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Agosto <- Pos_Mx[grep("-08-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Septiembre <- Pos_Mx[grep("-09-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Octubre <- Pos_Mx[grep("-10-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Noviembre <- Pos_Mx[grep("-11-",Pos_Mx$fecha_sintomas),]
Pos_Mx_Diciembre <- Pos_Mx[grep("-12-",Pos_Mx$fecha_sintomas),]

Pos_Mx_Enero$mes <- rep("Enero", length(Pos_Mx_Enero$fecha_actualizacion))
Pos_Mx_Febrero$mes <- rep("Febrero", length(Pos_Mx_Febrero$fecha_actualizacion))
Pos_Mx_Marzo$mes <- rep("Marzo", length(Pos_Mx_Marzo$fecha_actualizacion))
Pos_Mx_Abril$mes <- rep("Abril", length(Pos_Mx_Abril$fecha_actualizacion))
Pos_Mx_Mayo$mes <- rep("Mayo", length(Pos_Mx_Mayo$fecha_actualizacion))
Pos_Mx_Junio$mes <- rep("Junio", length(Pos_Mx_Junio$fecha_actualizacion))
Pos_Mx_Julio$mes <- rep("Julio", length(Pos_Mx_Julio$fecha_actualizacion))
Pos_Mx_Agosto$mes <- rep("Agosto", length(Pos_Mx_Agosto$fecha_actualizacion))
Pos_Mx_Septiembre$mes <- rep("Septiembre", length(Pos_Mx_Septiembre$fecha_actualizacion))
Pos_Mx_Octubre$mes <- rep("Octubre", length(Pos_Mx_Octubre$fecha_actualizacion))
Pos_Mx_Noviembre$mes <- rep("Noviembre", length(Pos_Mx_Noviembre$fecha_actualizacion))
Pos_Mx_Diciembre$mes <- rep("Diciembre", length(Pos_Mx_Diciembre$fecha_actualizacion))
Pos_Mx <- bind_rows(Pos_Mx_Enero, Pos_Mx_Febrero, Pos_Mx_Marzo, Pos_Mx_Abril,
                    Pos_Mx_Mayo, Pos_Mx_Junio, Pos_Mx_Julio, Pos_Mx_Agosto,
                    Pos_Mx_Septiembre, Pos_Mx_Octubre, Pos_Mx_Noviembre,
                    Pos_Mx_Diciembre)
rm(Pos_Mx_Enero, Pos_Mx_Febrero, Pos_Mx_Marzo, Pos_Mx_Abril,
   Pos_Mx_Mayo, Pos_Mx_Junio, Pos_Mx_Julio, Pos_Mx_Agosto,
   Pos_Mx_Septiembre, Pos_Mx_Octubre, Pos_Mx_Noviembre,
   Pos_Mx_Diciembre)
...
plot_defunciones_estados <- ggplot(mn_Def_Mx_Edos, aes(x = as.Date(fecha_def), y = casos, fill = tipo_paciente))+
  geom_bar(stat = "identity", binwidth = 0) +
  theme_classic()+
  facet_wrap(~state_name, scales = "free") +
  scale_fill_manual("Tipo de paciente", values = c("Hospitalizado" = "green", "Ambulatorio" = "gray")) + 
...
plot_Casos_Mx <- plot_ly(mn_Mx_2,
                          ...
                          type = 'bar',
                          colors = c("blue","gray")) %>% 
  layout(title = ~paste("<b>Casos positivos acumulados en México (n = </b>",n_Mx,"<b>)</b>"),
...
plot_Casos_Mx <- plot_ly(Decesos_Mx_2,
  ...
  layout(title = ~paste("<b>Fallecimientos en México (n = </b>",n_Mx,"<b>)</b>"),
         ...
         annotations = list(text = ~paste("<b>Hospitalizados:</b>", n_hosp,
                                          "<br><b>Ambulatorios:</b>", n_amb),
                            x = "2020-04-20", y = 650, showarrow=F,
                            font=list(color = "darkgreen")),
         bargap = 0,
         shapes = list(
           list(type = "rect",
...
marzo_tlax$mes <- rep("Marzo 2020", length(marzo_tlax$fecha_actualizacion))
...
abril_tlax$mes <- rep("Abril 2020", length(abril_tlax$fecha_actualizacion))
...
mayo_tlax$mes <- rep("Mayo 2020", length(mayo_tlax$fecha_actualizacion))
...
jun_tlax$mes <- rep("Junio 2020", length(jun_tlax$fecha_actualizacion))
...
jul_tlax$mes <- rep("Julio 2020", length(jul_tlax$fecha_actualizacion))
...
agos_tlax$mes <- rep("Agosto 2020", length(agos_tlax$fecha_actualizacion))
...
sept_tlax$mes <- rep("Septiembre 2020", length(sept_tlax$fecha_actualizacion))
...
oct_tlax$mes <- rep("Octubre 2020", length(oct_tlax$fecha_actualizacion))

nov_tlax <- Mex_Data_COVID[grep("-11-", Mex_Data_COVID$fecha_def),]
nov_tlax <- nov_tlax[grep("29",nov_tlax$entidad_res),]
nov_tlax <- nov_tlax[grep("Positivo", nov_tlax$value),]
nov_tlax$mes <- rep("Noviembre 2020", length(nov_tlax$fecha_actualizacion))

dic_tlax <- Mex_Data_COVID[grep("-12-", Mex_Data_COVID$fecha_def),]
dic_tlax <- dic_tlax[grep("29",dic_tlax$entidad_res),]
dic_tlax <- dic_tlax[grep("Positivo", dic_tlax$value),]
dic_tlax$mes <- rep("Diciembre 2020", length(dic_tlax$fecha_actualizacion))

# 2021
enero_tlax_2021 <- Mex_Data_COVID[grep("2021-01-", Mex_Data_COVID$fecha_def),]
enero_tlax_2021 <- enero_tlax_2021[grep("29",enero_tlax_2021$entidad_res),]
enero_tlax_2021 <- enero_tlax_2021[grep("Positivo", enero_tlax_2021$value),]
enero_tlax_2021$mes <- rep("Enero 2021", length(enero_tlax_2021$fecha_actualizacion))


def_mes_Tlx <- bind_rows(marzo_tlax, abril_tlax, mayo_tlax, jun_tlax, jul_tlax, agos_tlax, sept_tlax, oct_tlax, nov_tlax, dic_tlax, enero_tlax_2021)
def_mes_Tlx$mes <- factor(def_mes_Tlx$mes,  levels = c("Marzo 2020", "Abril 2020", "Mayo 2020", "Junio 2020", "Julio 2020", "Agosto 2020", "Septiembre 2020", "Octubre 2020", "Noviembre 2020", "Diciembre 2020", "Enero 2021"))
rm(marzo_tlax, abril_tlax, mayo_tlax, jun_tlax, jul_tlax, agos_tlax, sept_tlax, oct_tlax, nov_tlax, dic_tlax, enero_tlax_2021)

Def_edad_Tlx_meses_plot <- plot_ly(def_mes_Tlx, type = 'box') %>% 
  layout(boxmode = 'group',
         title = ~paste("<b>Distribución de fallecimientos por sexo y edad en Tlaxcala</b>"),
         titlefont = list(color = 'darkred'),
         yaxis = list(title = "Edad"),
         xaxis = list(title = "Mes")) %>% 
  add_trace(x =~mes[def_mes_Tlx$sexo == 'Hombre'],
            y =~edad[def_mes_Tlx$sexo == 'Hombre'],
            legendgroup = 'Hombre',
            scalegroup = 'Hombre',
            name = 'Hombre',
            box = list(visible = T),
            ...
            color = I("blue")) %>%
  add_trace(x =~mes[def_mes_Tlx$sexo == 'Mujer'],
            y =~edad[def_mes_Tlx$sexo == 'Mujer'],
            legendgroup = 'Mujer',
            scalegroup = 'Mujer',
            name = 'Mujer',
            box = list(visible = T),
...
#mnAct_Tlx <- ddply(activos_Tlx, c("municipio_name","region","municipio_code"), summarise,
 #               value = length(value))
#mnAct_Tlx$state_name <- rep("Tlaxcala",length(mnAct_Tlx))
#pal <- colorNumeric("Reds", domain = mnAct_Tlx$value)
